...
describe('Bitcoin Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, '../bin/eurekacoind');
  var nodesConf = [
    {
  ...
  before(function(done) {
    log.info('Starting 3 eurekacoind daemons');
    this.timeout(60000);
  ...
  it('step 1: will connect to three eurekacoind daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.eurekacoind.once('tip', function(height) {
