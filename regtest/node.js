...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.eurekacoind.height === 150) {
            node.services.eurekacoind.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.eurekacoind.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('eurekacoind/hashblock', function(data) {
        bus.unsubscribe('eurekacoind/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.eurekacoind.height;
      node.services.eurekacoind.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.eurekacoind.sendTransaction(tx.serialize(), function(err, hash) {
