...
var Transaction = eurekacoincore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/eurekacoin.conf')));
var EurekaCoinService = proxyquire('../../lib/services/eurekacoind', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultEurekaCoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.eurekacoin.conf'), 'utf8');

describe('EurekaCoin Service', function() {
	...
	var baseConfig = {
		node: {
			network: eurekacoincore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			should.exist(eurekacoind);
		});
		it('will create an instance without `new`', function() {
			var eurekacoind = EurekaCoinService(baseConfig);
			should.exist(eurekacoind);
		});
		it('will init caches', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);

			should.exist(eurekacoind.utxosCache);
			should.exist(eurekacoind.txidsCache);
			should.exist(eurekacoind.balanceCache);
			should.exist(eurekacoind.summaryCache);
			should.exist(eurekacoind.transactionDetailedCache);
			should.exist(eurekacoind.accountInfo);

			should.exist(eurekacoind.transactionCache);
			should.exist(eurekacoind.rawTransactionCache);
			should.exist(eurekacoind.rawJsonTransactionCache);
			should.exist(eurekacoind.blockCache);
			should.exist(eurekacoind.blockJsonCache);
			should.exist(eurekacoind.blockSubsidyCache);
			should.exist(eurekacoind.rawBlockCache);
			should.exist(eurekacoind.blockHeaderCache);
			should.exist(eurekacoind.zmqKnownTransactions);
			should.exist(eurekacoind.zmqKnownBlocks);
			should.exist(eurekacoind.lastTip);
			should.exist(eurekacoind.lastTipTimeout);
			should.equal(eurekacoind.dgpInfoCache, null, 'should be null');
			should.equal(eurekacoind.miningInfoCache, null, 'should be null');
			should.equal(eurekacoind.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(eurekacoind.maxTxids, 1000);
			should.equal(eurekacoind.maxTransactionHistory, 50);
			should.equal(eurekacoind.maxAddressesQuery, 10000);
			should.equal(eurekacoind.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(eurekacoind.spawnRestartTime, 5000);
			should.equal(eurekacoind.spawnStopTime, 10000);

			// try all interval
			should.equal(eurekacoind.tryAllInterval, 1000);
			should.equal(eurekacoind.startRetryInterval, 5000);

			// rpc limits
			should.equal(eurekacoind.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(eurekacoind.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.nodes.should.deep.equal([]);
			eurekacoind.nodesIndex.should.equal(0);
			eurekacoind.nodes.push({ client: sinon.stub() });
			should.exist(eurekacoind.client);
		});
		it('will set subscriptions', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind._initDefaults({ transactionConcurrency: 10 });
			eurekacoind.transactionConcurrency.should.equal(10);
			eurekacoind._initDefaults({});
			eurekacoind.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			EurekaCoinService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var methods = eurekacoind.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var events = eurekacoind.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('eurekacoind/rawtransaction');
			events[0].scope.should.equal(eurekacoind);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('eurekacoind/hashblock');
			events[1].scope.should.equal(eurekacoind);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('eurekacoind/addresstxid');
			events[2].scope.should.equal(eurekacoind);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('eurekacoind/addressbalance');
			events[3].scope.should.equal(eurekacoind);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.subscribe = sinon.stub();
			eurekacoind.unsubscribe = sinon.stub();
			var events = eurekacoind.getPublishEvents();

			events[0].subscribe('test');
			eurekacoind.subscribe.args[0][0].should.equal('rawtransaction');
			eurekacoind.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			eurekacoind.unsubscribe.args[0][0].should.equal('rawtransaction');
			eurekacoind.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			eurekacoind.subscribe.args[1][0].should.equal('hashblock');
			eurekacoind.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			eurekacoind.unsubscribe.args[1][0].should.equal('hashblock');
			eurekacoind.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var emitter = {};
			eurekacoind.subscribe('hashblock', emitter);
			eurekacoind.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			eurekacoind.subscribe('rawtransaction', emitter2);
			eurekacoind.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			eurekacoind.subscribe('hashblock', emitter1);
			eurekacoind.subscribe('hashblock', emitter2);
			eurekacoind.subscribe('hashblock', emitter3);
			eurekacoind.subscribe('hashblock', emitter4);
			eurekacoind.subscribe('hashblock', emitter5);
			eurekacoind.subscriptions.hashblock.length.should.equal(5);

			eurekacoind.unsubscribe('hashblock', emitter3);
			eurekacoind.subscriptions.hashblock.length.should.equal(4);
			eurekacoind.subscriptions.hashblock[0].should.equal(emitter1);
			eurekacoind.subscriptions.hashblock[1].should.equal(emitter2);
			eurekacoind.subscriptions.hashblock[2].should.equal(emitter4);
			eurekacoind.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			eurekacoind.subscriptions.hashblock = [emitter1];
			eurekacoind.unsubscribe('hashblock', emitter3);
			eurekacoind.subscriptions.hashblock.length.should.equal(1);
			eurekacoind.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			eurekacoind.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(eurekacoind.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var emitter = new EventEmitter();
			eurekacoind.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			eurekacoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			eurekacoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			eurekacoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.unsubscribeAddress(emitter1);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			eurekacoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(eurekacoind, 'unsubscribeAddressAll');
			eurekacoind.unsubscribeAddress(emitter1);
			eurekacoind.unsubscribeAddressAll.callCount.should.equal(1);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			eurekacoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			eurekacoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			eurekacoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			eurekacoind.unsubscribeAddressAll(emitter1);
			eurekacoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			eurekacoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			eurekacoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(eurekacoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var config = eurekacoind._getDefaultConfig();
			config.should.equal(defaultEurekaCoinConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a eurekacoin.conf file', function() {
			var TestEurekaCoin = proxyquire('../../lib/services/eurekacoind', {
				fs: {
					readFileSync: readFileSync,
			...
			var eurekacoind = new TestEurekaCoin(baseConfig);
			eurekacoind.options.spawn.datadir = '/tmp/.eurekacoin';

			sinon.spy(eurekacoind, '_expandRelativeDatadir');
			sinon.spy(eurekacoind, '_checkConfigIndexes');
			sinon.spy(eurekacoind, '_getDefaultConf')
			sinon.spy(eurekacoind, '_getNetworkConfigPath');
			sinon.spy(eurekacoind, '_parseBitcoinConf');

			var node = {};
			eurekacoind._loadSpawnConfiguration(node);

			eurekacoind._expandRelativeDatadir.callCount.should.equal(1);
			eurekacoind._checkConfigIndexes.callCount.should.equal(1);
			eurekacoind._getDefaultConf.callCount.should.equal(1);
			eurekacoind._getNetworkConfigPath.callCount.should.equal(1);
			eurekacoind._parseBitcoinConf.callCount.should.equal(2);

			should.exist(eurekacoind.spawn.config);
			eurekacoind.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: eurekacoincore.Networks.testnet,
					configPath: '/tmp/.eurekacoincore/eurekacoincore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var eurekacoind = new TestEurekaCoin(config);
			...
			eurekacoind.options.spawn.datadir = './data';
			var node = {};
			eurekacoind._loadSpawnConfiguration(node);
			...
			eurekacoind.options.spawn.datadir.should.equal('/tmp/.eurekacoincore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestEurekaCoin = proxyquire('../../lib/services/eurekacoind', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badeurekacoin.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var eurekacoind = new TestEurekaCoin(baseConfig);
			(function() {
				eurekacoind._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(eurekacoincore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the eurekacoin.conf file', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				eurekacoind._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the eurekacoin.conf file', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the eurekacoin.conf file', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the eurekacoin.conf file', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the eurekacoin.conf file', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				eurekacoind.transactionDetailedCache.set(keys[i], {});
				eurekacoind.utxosCache.set(keys[i], {});
				eurekacoind.accountInfo.set(keys[i], {});
				eurekacoind.txidsCache.set(keys[i], {});
				eurekacoind.balanceCache.set(keys[i], {});
				eurekacoind.summaryCache.set(keys[i], {});
				eurekacoind.blockOverviewCache.set(keys[i], {});
				eurekacoind.blockJsonCache.set(keys[i], {});
			}
			eurekacoind._resetCaches();
			should.equal(eurekacoind.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(eurekacoind.utxosCache.get(keys[0]), undefined);
			should.equal(eurekacoind.accountInfo.get(keys[0]), undefined);
			should.equal(eurekacoind.txidsCache.get(keys[0]), undefined);
			should.equal(eurekacoind.balanceCache.get(keys[0]), undefined);
			should.equal(eurekacoind.summaryCache.get(keys[0]), undefined);
			should.equal(eurekacoind.blockOverviewCache.get(keys[0]), undefined);
			should.equal(eurekacoind.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.tryAllInterval = 1;
			eurekacoind.nodes.push({
				client: {
			...
			eurekacoind._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				eurekacoind.nodes[0].client.getInfo.callCount.should.equal(1);
				eurekacoind.nodes[1].client.getInfo.callCount.should.equal(1);
				eurekacoind.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			eurekacoind._tryAllClients(function(client, next) {
				...
				eurekacoind.nodes[1].client.getInfo.callCount.should.equal(1);
				eurekacoind.nodes[2].client.getInfo.callCount.should.equal(1);
				eurekacoind.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert eurekacoind-rpc error object into JavaScript error', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var error = eurekacoind._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			eurekacoind.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			eurekacoind.nodes.push({
				client: {
			...
			eurekacoind._initChain(function() {
				log.info.callCount.should.equal(1);
				eurekacoind.getRawBlock.callCount.should.equal(1);
				eurekacoind.getRawBlock.args[0][0].should.equal('genesishash');
				eurekacoind.height.should.equal(5000);
				eurekacoind.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			eurekacoind._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			eurekacoind.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			eurekacoind._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			eurekacoincore.Networks.disableRegtest();
			baseConfig.node.network = eurekacoincore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: eurekacoincore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var eurekacoind = new EurekaCoinService(config);
			eurekacoind._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var eurekacoind = new EurekaCoinService(config);
			eurekacoind._getDefaultConf().rpcport.should.equal(18332);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var eurekacoind = new EurekaCoinService(config);
			should.equal(eurekacoind._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var eurekacoind = new EurekaCoinService(config);
			eurekacoind._getNetworkConfigPath().should.equal('testnet3/eurekacoin.conf');
		});
		it('will get default rpc port for regtest', function() {
			eurekacoincore.Networks.enableRegtest();
			var config = {
			...
			var eurekacoind = new EurekaCoinService(config);
			eurekacoind._getNetworkConfigPath().should.equal('regtest/eurekacoin.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.node.network = eurekacoincore.Networks.testnet;
			eurekacoind._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.node.network = eurekacoincore.Networks.testnet;
			eurekacoincore.Networks.enableRegtest();
			eurekacoind._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.node.network = eurekacoincore.Networks.livenet;
			eurekacoincore.Networks.enableRegtest();
			should.equal(eurekacoind._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			eurekacoind._rapidProtectedUpdateTip = sinon.stub();
			eurekacoind.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			eurekacoind._zmqBlockHandler(node, message);
			eurekacoind._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			eurekacoind._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(eurekacoind, 'emit');
			eurekacoind.on('block', function(block) {
				block.should.equal(message);
				eurekacoind.emit.callCount.should.equal(1);
				eurekacoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			eurekacoind._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			eurekacoind._zmqBlockHandler(node, message);
			eurekacoind._rapidProtectedUpdateTip.callCount.should.equal(1);
			eurekacoind._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			eurekacoind._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(eurekacoind, 'emit');
			var emitter = new EventEmitter();
			eurekacoind.subscriptions.hashblock.push(emitter);
			emitter.on('eurekacoind/hashblock', function(blockHash) {
				eurekacoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				eurekacoind.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var callCount = 0;
			eurekacoind._updateTip = function() {
				callCount++;
			...
			var count = 0;
			function repeat() {
				eurekacoind._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub();
			sinon.spy(eurekacoind, '_resetCaches');
			eurekacoind.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				eurekacoind._resetCaches.callCount.should.equal(1);
			...
			eurekacoind._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			eurekacoind.on('synced', function() {
				eurekacoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			eurekacoind.on('synced', function() {
				eurekacoind.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			eurekacoind.on('error', function(err) {
				log.error.callCount.should.equal(1);
				eurekacoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub();
			eurekacoind._resetCaches = sinon.stub();
			eurekacoind.on('tip', function(height) {
				eurekacoind._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				eurekacoind.height.should.equal(10);
				done();
			});
			var node = {
				client: {
		...
		it('will not call syncPercentage if node is stopping', function(done) {
			...
			eurekacoind.syncPercentage = sinon.stub();
			eurekacoind._resetCaches = sinon.stub();
			eurekacoind.node.stopping = true;
			var node = {
				client: {
					getBlock: sinon.stub().callsArgWith(1, null, {
						result: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using eurekacoincore.Transaction', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = eurekacoincore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(eurekacoincore.Networks.testnet);
			var outputAddress = eurekacoincore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = eurekacoincore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: eurekacoincore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = eurekacoind._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var tx = eurekacoincore.Transaction();
			tx.addInput(eurekacoincore.Transaction.Input({
				prevTxId: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				script: eurekacoincore.Script('OP_TRUE'),
				outputIndex: 1,
				output: {
					script: eurekacoincore.Script('OP_TRUE'),
					satoshis: 5000000000
			...
			tx.addOutput(eurekacoincore.Transaction.Output({
				script: eurekacoincore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = eurekacoind._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var tx = eurekacoincore.Transaction();
			var address = eurekacoincore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(eurekacoincore.Transaction.Output({
				script: eurekacoincore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(eurekacoincore.Transaction.Output({
				script: eurekacoincore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			eurekacoind._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			eurekacoind.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('eurekacoind/addresstxid', function(data) {
			...
			eurekacoind._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			eurekacoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('eurekacoind/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			eurekacoind._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(eurekacoind, 'emit');
			eurekacoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('eurekacoind/rawtransaction', function() {
				done();
			});
			var node = {};
			eurekacoind._zmqTransactionHandler(node, expectedBuffer);
		...
		it('will emit "tx" event', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var expectedBuffer = new Buffer(txhex, 'hex');
			eurekacoind.on('tx', function(buffer) {
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind._updateTip = sinon.stub();
			eurekacoind._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			eurekacoind.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			eurekacoind._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				eurekacoind._updateTip.callCount.should.equal(11);
				eurekacoind._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			eurekacoind._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				eurekacoind._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			eurekacoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				eurekacoind._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			eurekacoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				eurekacoind._zmqBlockHandler.callCount.should.equal(0);
				eurekacoind._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var eurekacoind = new EurekaCoinService(baseConfig);
			var node = {};
			eurekacoind._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			eurekacoind._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			eurekacoind._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			eurekacoind._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			eurekacoind.on('tip', function(height) {
				...
				eurekacoind.height.should.equal(100);
				done();
			});
			eurekacoind._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestEurekaCoinService = proxyquire('../../lib/services/eurekacoind', {
				fs: {
					readFile: readFile
				}
			});
			var eurekacoind = new TestEurekaCoinService(baseConfig);
			eurekacoind.spawnStopTime = 1;
			eurekacoind._process = {};
			eurekacoind._process.kill = sinon.stub();
			eurekacoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				eurekacoind._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			eurekacoind._process.kill = sinon.stub().throws(error2);
			eurekacoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind._loadSpawnConfiguration = sinon.stub();
			eurekacoind._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			eurekacoind._spawnChildProcess(function(err) {
				eurekacoind._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind._loadSpawnConfiguration = sinon.stub();
			eurekacoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			eurekacoind._spawnChildProcess(function(err) {
				eurekacoind._loadSpawnConfiguration.callCount.should.equal(1);
				eurekacoind._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var eurekacoind = new TestEurekaCoinService(config);
			eurekacoind.spawn = {};
			eurekacoind._loadSpawnConfiguration = sinon.stub();
			eurekacoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			eurekacoind.node.stopping = true;
			eurekacoind._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			eurekacoind._loadSpawnConfiguration = sinon.stub();
			eurekacoind.spawn = {};
			eurekacoind.spawn.exec = 'testexec';
			eurekacoind.spawn.configPath = 'testdir/eurekacoin.conf';
			eurekacoind.spawn.datadir = 'testdir';
			eurekacoind.spawn.config = {};
			eurekacoind.spawn.config.rpcport = 20001;
			eurekacoind.spawn.config.rpcuser = 'eurekacoin';
			eurekacoind.spawn.config.rpcpassword = 'password';
			eurekacoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			eurekacoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			eurekacoind._initZmqSubSocket = sinon.stub();
			eurekacoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			eurekacoind._checkReindex = sinon.stub().callsArgWith(1, null);
			eurekacoind._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/eurekacoin.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				eurekacoind._loadTipFromNode.callCount.should.equal(1);
				eurekacoind._initZmqSubSocket.callCount.should.equal(1);
				should.exist(eurekacoind._initZmqSubSocket.args[0][0].client);
				eurekacoind._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				eurekacoind._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(eurekacoind._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn eurekacoind spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestEurekaCoinService = proxyquire('../../lib/services/eurekacoind', {
				fs: {
			...
			eurekacoind._loadSpawnConfiguration = sinon.stub();
			eurekacoind.spawn = {};
			eurekacoind.spawn.exec = 'eurekacoind';
			eurekacoind.spawn.datadir = '/tmp/eurekacoin';
			eurekacoind.spawn.configPath = '/tmp/eurekacoin/eurekacoin.conf';
			eurekacoind.spawn.config = {};
			eurekacoind.spawnRestartTime = 1;
			eurekacoind._loadTipFromNode = sinon.stub().callsArg(1);
			eurekacoind._initZmqSubSocket = sinon.stub();
			eurekacoind._checkReindex = sinon.stub().callsArg(1);
			eurekacoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			eurekacoind._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(eurekacoind, '_spawnChildProcess');
			eurekacoind._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						eurekacoind._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			eurekacoind._spawnChildProcess(function(err) {
				...
				eurekacoind._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				eurekacoind.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn eurekacoind spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestEurekaCoinService = proxyquire('../../lib/services/eurekacoind', {
				fs: {
			...
			eurekacoind._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						eurekacoind._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			var eurekacoind = new TestEurekaCoinService(baseConfig);
			eurekacoind.startRetryInterval = 1;
			eurekacoind._loadSpawnConfiguration = sinon.stub();
			eurekacoind.spawn = {};
			eurekacoind.spawn.exec = 'testexec';
			eurekacoind.spawn.configPath = 'testdir/eurekacoin.conf';
			...
			eurekacoind.spawn.config.rpcpassword = 'password';
			eurekacoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			eurekacoind._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			eurekacoind._spawnChildProcess(function(err) {
				eurekacoind._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			eurekacoind._initZmqSubSocket = sinon.stub();
			eurekacoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			eurekacoind._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var eurekacoind = new EurekaCoinService(config);
			eurekacoind.node.stopping = true;
			eurekacoind.startRetryInterval = 100;
			eurekacoind._loadTipFromNode = sinon.stub();
			eurekacoind._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to eurekacoind/);
				eurekacoind._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			eurekacoind.startRetryInterval = 1;
			var config = {};
			eurekacoind._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				eurekacoind._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			eurekacoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			eurekacoind._connectProcess(config, function(err, node) {
				should.not.exist(err);
				eurekacoind._loadTipFromNode.callCount.should.equal(1);
				eurekacoind._initZmqSubSocket.callCount.should.equal(1);
				eurekacoind._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.options = {};
			eurekacoind.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('EurekaCoin configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			eurekacoind.options = {
				connect: [
					{}
				]
			...
			eurekacoind.start(function(err) {
				eurekacoind._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var node = {};
			eurekacoind._initChain = sinon.stub().callsArg(0);
			eurekacoind._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			eurekacoind.options = {
				spawn: {}
			};
			eurekacoind.start(function(err) {
				should.not.exist(err);
				eurekacoind.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(eurekacoind, '_spawnChildProcess');
			var nodes = [{}];
			eurekacoind._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			eurekacoind.options = {
				connect: [
					{}
				]
			...
			eurekacoind.start(function(err) {
				should.not.exist(err);
				eurekacoind._initChain.callCount.should.equal(1);
				eurekacoind._connectProcess.callCount.should.equal(1);
				eurekacoind.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		it('will give error from syncPercentage', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			eurekacoind.isSynced(function(err) {
				should.exist(err);
				eurekacoind.syncPercentage.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			eurekacoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				eurekacoind.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			eurekacoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			eurekacoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			eurekacoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.syncPercentage(function(err) {
				should.exist(err);
				eurekacoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			eurekacoind.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				eurekacoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var args = eurekacoind._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var args = eurekacoind._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			eurekacoind.nodes.push({
				client: {
			...
			var options = {};
			eurekacoind.getAddressBalance(address, options, function(err) {
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			eurekacoind.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				eurekacoind.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					eurekacoind._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			eurekacoind.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = eurekacoind._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = eurekacoind._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = eurekacoind._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = eurekacoind._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				eurekacoind._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			eurekacoind.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(eurekacoind, '_getHeightRangeQuery');
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			eurekacoind.getAddressTxids(address, options, function(err) {
				should.exist(err);
				eurekacoind._getHeightRangeQuery.callCount.should.equal(1);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			eurekacoind.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				eurekacoind._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			eurekacoind.getAddressTxids(address, options, function(err, txids) {
				...
				eurekacoind.getAddressTxids(address, options, function(err, txids) {
					...
					eurekacoind._getHeightRangeQuery.callCount.should.equal(2);
					eurekacoind._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			eurekacoind.getAddressTxids(address, options, function(err, txids) {
				...
				eurekacoind.getAddressTxids(address, options, function(err, txids) {
					...
					eurekacoind._getHeightRangeQuery.callCount.should.equal(4);
					eurekacoind._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			eurekacoind.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				eurekacoind.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					eurekacoind.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						eurekacoind._getHeightRangeQuery.callCount.should.equal(3);
						eurekacoind._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			tx.height = -1;
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.height = 10;
			var confirmations = eurekacoind._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var result = {};
			eurekacoind._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var result = {};
			eurekacoind._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var result = {};
			eurekacoind._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var result = {};
			eurekacoind._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(eurekacoind, '_getAddressDetailsForInput');
			sinon.spy(eurekacoind, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = eurekacoind._getAddressDetailsForTransaction(tx, addresses);
			...
			eurekacoind._getAddressDetailsForInput.callCount.should.equal(1);
			eurekacoind._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_getConfirmationsDetail');
			eurekacoind.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			eurekacoind.height = 300;
			var addresses = {};
			eurekacoind._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			eurekacoind._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				eurekacoind.getDetailedTransaction.callCount.should.equal(1);
				eurekacoind._getAddressDetailsForTransaction.callCount.should.equal(1);
				eurekacoind._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			eurekacoind._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				eurekacoind.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from eurekacoincore addresses', function() {
			var addresses = [
				eurekacoincore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				eurekacoincore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var eurekacoind = new EurekaCoinService(baseConfig);
			var strings = eurekacoind._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var eurekacoind = new EurekaCoinService(baseConfig);
			(function() {
				eurekacoind._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			eurekacoind.maxAddressesQuery = 100;
			eurekacoind.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			sinon.spy(eurekacoind, '_getAddressStrings');
			eurekacoind.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				eurekacoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			sinon.spy(eurekacoind, '_getAddressStrings');
			eurekacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			eurekacoind.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				eurekacoind._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			sinon.spy(eurekacoind, '_getAddressStrings');
			eurekacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			eurekacoind._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			eurekacoind.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				eurekacoind._getAddressStrings.callCount.should.equal(1);
				eurekacoind.getAddressTxids.callCount.should.equal(1);
				eurekacoind._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			sinon.spy(eurekacoind, '_getAddressStrings');
			sinon.spy(eurekacoind, 'getAddressTxids');
			eurekacoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			eurekacoind.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				eurekacoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			sinon.spy(eurekacoind, '_getAddressStrings');
			sinon.spy(eurekacoind, '_paginateTxids');
			eurekacoind._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(eurekacoind, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			eurekacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			eurekacoind.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				eurekacoind._getAddressStrings.callCount.should.equal(1);
				eurekacoind.getAddressTxids.callCount.should.equal(1);
				eurekacoind._getAddressDetailedTransaction.callCount.should.equal(2);
				eurekacoind._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			eurekacoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			eurekacoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			eurekacoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			eurekacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			eurekacoind.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			eurekacoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			eurekacoind.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			eurekacoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			eurekacoind.getAddressSummary(address, options, function(err, summary) {
				eurekacoind._paginateTxids.callCount.should.equal(1);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				eurekacoind.getAddressTxids.callCount.should.equal(1);
				eurekacoind.getAddressBalance.callCount.should.equal(1);
				eurekacoind._paginateTxids.args[0][1].should.equal(0);
				eurekacoind._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			eurekacoind.getAddressSummary(address, options, function(err, summary) {
				...
				eurekacoind.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					eurekacoind._paginateTxids.callCount.should.equal(0);
					eurekacoind._normalizeAddressArg.callCount.should.equal(2);
					eurekacoind.getAddressTxids.callCount.should.equal(1);
					eurekacoind.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			eurekacoind.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				eurekacoind._paginateTxids.callCount.should.equal(1);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				eurekacoind.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			eurekacoind._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			eurekacoind.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(eurekacoind, '_tryAllClients');
			eurekacoind.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			eurekacoind.getRawBlock(blockhash, function(err) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			eurekacoind.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			eurekacoind.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			eurekacoind.getBlock(0, function(err) {
				eurekacoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as eurekacoincore object from height', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			eurekacoind.getBlock(0, function(err, block) {
				should.not.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(eurekacoincore.Block);
				done();
			});
		});
		it('will getblock as eurekacoincore object', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			eurekacoind.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			eurekacoind.getBlock(hash, function(err, block) {
				should.not.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			eurekacoind.getBlock(0, function(err, block) {
				...
				eurekacoind.getBlock(0, function(err, block) {
					should.not.exist(err);
					eurekacoind._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			eurekacoind.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			eurekacoind.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			eurekacoind.getBlockHeader(10, function(err) {
				eurekacoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			eurekacoind.getBlockHeader(blockhash, function(err) {
				eurekacoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			eurekacoind.getBlockHeader(0, function(err) {
				eurekacoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			eurekacoind.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				eurekacoind._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			eurekacoind.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				eurekacoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			eurekacoind._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				eurekacoind._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			eurekacoind._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			eurekacoind._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_maybeGetBlockHash');
			eurekacoind._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			eurekacoind.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				eurekacoind._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			eurekacoind.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				eurekacoind.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					eurekacoind._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			eurekacoind.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = eurekacoincore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			eurekacoind.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			eurekacoind.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			eurekacoind.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = eurekacoincore.Transaction();
			(function() {
				eurekacoind.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getRawTransaction('txid', function(err) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			eurekacoind.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getTransaction('txid', function(err) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			eurekacoind.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(eurekacoincore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			eurekacoind.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			eurekacoind.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			eurekacoind.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			eurekacoind.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getInfo(function(err) {
				should.exist(err);
				err.should.be.an.instanceof(errors.RPCError);
				done();
			});
		...
		it('will call client getInfo and give result', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.node.getNetworkName = sinon.stub().returns('testnet');
			...
			eurekacoind.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			eurekacoind.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			eurekacoind.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				eurekacoind.getSubsidy.callCount.should.equal(1);
				eurekacoind.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			eurekacoind.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		it('will callback if spawn is not set', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.stop(done);
		});
		it('will exit spawned process', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.spawn = {};
			eurekacoind.spawn.process = new EventEmitter();
			eurekacoind.spawn.process.kill = sinon.stub();
			eurekacoind.stop(done);
			eurekacoind.spawn.process.kill.callCount.should.equal(1);
			eurekacoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			eurekacoind.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			eurekacoind.spawn.process.kill.callCount.should.equal(1);
			eurekacoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			eurekacoind.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var eurekacoind = new EurekaCoinService(baseConfig);
			eurekacoind.shutdownTimeout = 300;
			eurekacoind.spawn = {};
			eurekacoind.spawn.process = new EventEmitter();
			eurekacoind.spawn.process.kill = sinon.stub();
			eurekacoind.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_normalizeAddressArg');
			sinon.spy(eurekacoind, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			eurekacoind.nodes.push({
			...
			eurekacoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				eurekacoind._getBalanceFromMempool.callCount.should.equal(1);
				eurekacoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			eurekacoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			eurekacoind.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			eurekacoind.getJsonBlock(0, function(err) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			eurekacoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			eurekacoind.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(2);
				eurekacoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			eurekacoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
				eurekacoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var emitter = new EventEmitter();
			eurekacoind.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(eurekacoind.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			var emitter = new EventEmitter();
			eurekacoind.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			eurekacoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			eurekacoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			eurekacoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.unsubscribeBalance(emitter1);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			eurekacoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			...
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			eurekacoind.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var eurekacoind = new EurekaCoinService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(eurekacoind, 'unsubscribeBalanceAll');
			eurekacoind.unsubscribeBalance(emitter1);
			eurekacoind.unsubscribeBalanceAll.callCount.should.equal(1);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			eurekacoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			eurekacoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			eurekacoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			eurekacoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			eurekacoind.unsubscribeBalanceAll(emitter1);
			eurekacoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			eurekacoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			eurekacoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(eurekacoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var eurekacoind = new EurekaCoinService(baseConfig);
			sinon.spy(eurekacoind, '_notifyBalanceSubscriber');
			eurekacoind._getAddressesFromTransaction = sinon.stub().returns(addresses);
			eurekacoind.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				eurekacoind._notifyBalanceSubscriber.callCount.should.equal(1);
				eurekacoind._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			eurekacoind._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				eurekacoind.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			eurekacoind._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			eurekacoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getDgpInfo(function(err, result) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getMiningInfo(function(err, result) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			eurekacoind.getStakingInfo(function(err, result) {
				should.exist(err);
				eurekacoind._tryAllClients.callCount.should.equal(1);
