...
var EurekaCoinService = require('../../lib/services/eurekacoind');
...
describe('#start', function() {
  ...
  describe('will dynamically create a node from a configuration', function() {
    ...
    it('require each eurekacoincore-node service with default config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'eurekacoind',
          module: EurekaCoinService,
          config: {
            spawn: {
              datadir: './data'
            }
    ...
    it('require each eurekacoincore-node service with explicit config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'eurekacoind',
