...
var bitcore = require('eurekacoincore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var EurekaCoinRPC = require('eurekacoind-rpc');
...
/**
 * Provides a friendly event driven API to eurekacoind in Node.js. Manages starting and
 * stopping eurekacoind as a child process for application support, as well as connecting
 * to multiple eurekacoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function EurekaCoin(options) {
  if (!(this instanceof EurekaCoin)) {
    return new EurekaCoin(options);
  }
...
util.inherits(EurekaCoin, Service);

EurekaCoin.dependencies = [];

EurekaCoin.DEFAULT_MAX_TXIDS = 1000;
EurekaCoin.DEFAULT_MAX_HISTORY = 50;
EurekaCoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
EurekaCoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
EurekaCoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
EurekaCoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
EurekaCoin.DEFAULT_SPAWN_STOP_TIME = 10000;
EurekaCoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
EurekaCoin.DEFAULT_REINDEX_INTERVAL = 10000;
EurekaCoin.DEFAULT_START_RETRY_INTERVAL = 5000;
EurekaCoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
EurekaCoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
EurekaCoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
EurekaCoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || EurekaCoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || EurekaCoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || EurekaCoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || EurekaCoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || EurekaCoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || EurekaCoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || EurekaCoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || EurekaCoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || EurekaCoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || EurekaCoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

EurekaCoin.prototype._initCaches = function() {
...
EurekaCoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
EurekaCoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
EurekaCoin.prototype.getPublishEvents = function() {
  return [
    {
...
EurekaCoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'eurekacoind/' + name, 'total:', this.subscriptions[name].length);
};

EurekaCoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'eurekacoind/' + name, 'total:', this.subscriptions[name].length);
};


EurekaCoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
EurekaCoin.prototype.unsubscribeBalance = function(emitter, addresses) {
...
EurekaCoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
EurekaCoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'eurekacoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

EurekaCoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'eurekacoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
EurekaCoin.prototype.unsubscribeAddressAll = function(emitter) {
...
EurekaCoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = EurekaCoin.DEFAULT_CONFIG_SETTINGS;
...
EurekaCoin.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
EurekaCoin.prototype._expandRelativeDatadir = function() {
...
EurekaCoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in eurekacoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in eurekacoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in eurekacoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './eurekacoin.conf');

  log.info('Using eurekacoin config file:', configPath);
...
EurekaCoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of eurekacoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of eurekacoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of eurekacoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to eurekacoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from eurekacoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from eurekacoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that eurekacoind is undergoing a reindex. ' +
             ...
             'of eurekacoincore-node services will start.');
    node._reindex = true;
  }
};

EurekaCoin.prototype._resetCaches = function() {
...
EurekaCoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
EurekaCoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

EurekaCoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('EurekaCoin Daemon Ready');
          callback();
        });
      });
...
EurekaCoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
EurekaCoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/eurekacoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/eurekacoin.conf';
    }
  }
  return networkPath;
};

EurekaCoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
EurekaCoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('eurekacoind/hashblock', message.toString('hex'));
    }
  }

};

EurekaCoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
EurekaCoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('EurekaCoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
EurekaCoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
EurekaCoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
EurekaCoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
EurekaCoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('eurekacoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
EurekaCoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('eurekacoind/rawtransaction', message.toString('hex'));
    }
...
EurekaCoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || EurekaCoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

EurekaCoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
EurekaCoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
EurekaCoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('EurekaCoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || EurekaCoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

EurekaCoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
EurekaCoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/eurekacoind.pid';
...
EurekaCoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting eurekacoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('EurekaCoin process unexpectedly exited with code:', code);
        log.warn('Restarting eurekacoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('EurekaCoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn eurekacoind.'));
      }
...
EurekaCoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to eurekacoind.'));
    }
 ...
 */
EurekaCoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('EurekaCoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
EurekaCoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, eurekacoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
EurekaCoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
EurekaCoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
EurekaCoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
EurekaCoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in eurekacoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
EurekaCoin.prototype._getConfirmationsDetail = function(transaction) {
...
EurekaCoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
EurekaCoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
EurekaCoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A eurekacoincoin transaction id
 * @param {Function} callback
 */
EurekaCoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
EurekaCoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
EurekaCoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
EurekaCoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
EurekaCoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a EurekaCoincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
EurekaCoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getInfo = function(callback) {
...
EurekaCoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('eurekacoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('eurekacoind process did not exit'));
      }
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
EurekaCoin.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
EurekaCoin.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
EurekaCoin.prototype.getStakingInfo = function(callback) {
    var self = this,
