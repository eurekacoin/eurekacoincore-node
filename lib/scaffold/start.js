...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for eurekacoind is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and eurekacoincore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed eurekacoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './eurekacoincore-node.json');
  ...
  node.on('ready', function() {
    log.info('EurekaCoincore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in eurekacoincore-node services directory
    ...
    if (servicePackage.eurekacoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.eurekacoincoreNode;
    }
