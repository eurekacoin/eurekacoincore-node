...
/**
 * Will return the path and default eurekacoincore-node configuration. It will search for the
...
function getDefaultConfig(options) {
  ...
  var defaultConfigFile = path.resolve(defaultPath, './eurekacoincore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['eurekacoind', 'web'];
