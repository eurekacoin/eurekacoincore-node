...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'eurekacoincore-lib': 'https://github.com/eurekacoin/eurekacoincore-lib.git#master',
    'eurekacoincore-node': 'https://github.com/eurekacoin/eurekacoincore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/eurekacoincore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the eurekacoincore-node directory and configuration
